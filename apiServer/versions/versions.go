...
/*
	请求url需要拼接,通过es的api去构造url，返回的是json，解析到结构体，然后输出即可
...
func AllGet(ctx *gin.Context) {
	...
	result, err := GetAllVersions(bucket, name)
	if err != nil {
		log.Println(err)
...
// GetAllVersions 获取所有对象最新版本
func GetAllVersions(bucket string, name string) ([]es.Metadata, error) {
	from := 0
	size := 1000
