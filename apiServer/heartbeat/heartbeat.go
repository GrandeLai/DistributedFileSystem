...
// ListenHeartbeat 监听apiServers，将数据服务节点的地址保存起来
func ListenHeartbeat() {
	...
	for {
		//接收所有dataServer节点发生的heartbeat信息
		msg, err := pubsub.ReceiveMessage(context.Background())
		if err != nil {
			golog.Error.Println("redis receive msg err:", err)
		}
	...
	for {
		...
		for dataServerAddr, timeStamp := range dataServersMap {
			...
			if timeStamp.Add(10 * time.Second).Before(time.Now()) {
				...
				golog.Warn.Println(fmt.Sprintf("检测到 dataServer:%s 节点下线", dataServerAddr))
				//TODO 扩展-邮件通知/群通知
			}
		}
		mutex.Unlock()
...
// GetDataServers 以切片的形式返回所有活跃的数据节点地址
