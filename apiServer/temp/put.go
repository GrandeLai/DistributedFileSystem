...
import (
	...
	"DistributedFileSystem/common/utils"
...
func Put(ctx *gin.Context) {
	...
	for {
		n, err := io.ReadFull(r.Body, bytes) //从r.Body中读取len(bytes)字节到bytes中，返回复制的字节数和一个错误
		...
		if current == stream.Size { //如果读到的总长度等于对象的大小，说明客户端上传了全部数据
			...
			rdb := RedisMQ.Rds
			rdb.Incr("DFS-Echarts" + time.Now().Format("2006-01-02"))
			if err != nil {
